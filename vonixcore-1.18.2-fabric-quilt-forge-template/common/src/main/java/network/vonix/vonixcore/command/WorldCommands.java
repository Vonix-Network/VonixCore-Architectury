...
public class WorldCommands {
    ...
    private static int setWeather(CommandContext<CommandSourceStack> ctx, String type, int duration) {
        ...
        switch (type) {
            ...
            case "storm" -> {
                ...
                ctx.getSource().sendSuccess(new TextComponent("§aWeather set to storm"), true);
            }
        }
        return 1;
    }
    ...
    private static int setTime(CommandContext<CommandSourceStack> ctx, int ticks) {
        ...
        ctx.getSource().sendSuccess(new TextComponent("§aTime set to §e" + ticks), true);
        return 1;
    }
    ...
    private static int addTime(CommandContext<CommandSourceStack> ctx, int ticks) {
        ...
        ctx.getSource().sendSuccess(new TextComponent("§aAdded §e" + ticks + "§a ticks to time"), true);
        return 1;
    }
    ...
    private static int lightningAtPlayer(CommandContext<CommandSourceStack> ctx) {
        ...
        player.sendMessage(new TextComponent("§eLightning struck at your location!"), player.getUUID());
        return 1;
    }
    ...
    private static int lightningAtTarget(CommandContext<CommandSourceStack> ctx, ServerPlayer target) {
        ...
        ctx.getSource().sendSuccess(
                new TextComponent("§eStruck §6" + target.getName().getString() + "§e with lightning!"), true);
        target.sendMessage(new TextComponent("§cYou were struck by lightning!"), target.getUUID());
        return 1;
    }
    ...
    private static int extinguishSelf(CommandContext<CommandSourceStack> ctx) {
        if (!(ctx.getSource().getEntity() instanceof ServerPlayer player))
            return 0;
        player.clearFire();
        player.sendMessage(new TextComponent("§aYou have been extinguished"), player.getUUID());
        return 1;
    }
    ...
    private static int extinguishPlayer(CommandContext<CommandSourceStack> ctx, ServerPlayer target) {
        ...
        target.sendMessage(new TextComponent("§aYou have been extinguished"), target.getUUID());
        return 1;
    }
    ...
    private static int toggleAfk(CommandContext<CommandSourceStack> ctx, String message) {
        ...
        if (afkTime.containsKey(uuid)) {
            ...
            player.sendMessage(new TextComponent("§aYou are no longer AFK"), player.getUUID());
        } else {
            // Go AFK
            ...
            player.sendMessage(new TextComponent("§eYou are now AFK" + (message != null ? ": " + message : "")), player.getUUID());
        }
        return 1;
    }
