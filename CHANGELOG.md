...
### Added
- **Auth Freeze Enforcement Mixins** (All Versions):
    - `AuthCommandBlockerMixin`: Blocks commands for unauthenticated/frozen players (allows /login, /register)
    - `AuthEntityInteractionMixin`: Prevents entity interaction while frozen
    - `AuthItemPickupMixin`: Prevents item pickup while frozen
    - Added to 1.20.1 fabric + forge (was completely missing)
    - Added to 1.18.2 forge (fabric already had them)
    - 1.19.2 and 1.21.1 already had complete coverage

### Fixed
- **1.20.1 Auth Initialization**: Added missing `AuthEventHandler.init()` and `AuthenticationManager.updateFreezeCache()` calls in `VonixCore.java`
- **Production Cleanup**: Removed debug log statements from 1.20.1 forge `ServerGamePacketListenerMixin`

### Changed
- **Version Numbers** (Semantic Versioning): All versions bumped `1.3.2` → `1.4.0` (minor bump for new auth freeze feature)
- **Repository Cleanup**: Removed temporary diff_*.txt files from repository root
