...
/**
 * CoreProtect-style protection commands for Fabric 1.20.1.
...
public class ProtectionCommands {
    ...
    private static int lookupWithParams(CommandContext<CommandSourceStack> ctx, String params) {
        ...
        if (parsed.radius <= 0)
            parsed.radius = ProtectionConfig.getInstance().getMaxRadius();
        if (parsed.time <= 0)
            parsed.time = ProtectionConfig.getInstance().getMaxTimeDays() * 86400L;

        BlockPos playerPos = player.blockPosition();
        String world = player.level().dimension().location().toString();
        int maxResults = 50; // Default max lookup results
        ...
        CompletableFuture.runAsync(() -> {
            try (Connection conn = VonixCore.getInstance().getDatabase().getConnection()) {
                ...
                ctx.getSource().getServer().execute(() -> {
                    ...
                    sendSuccess(ctx.getSource(), Component.literal("§7Queue size: §fN/A"));
                });
