...
public class LinkedAccountsManager {
    ...
    private static final String LINKED_ACCOUNTS_FILE = "vonixcore-discord-links.json";
    ...
    public String generateLinkCode(UUID minecraftUUID, String minecraftUsername) {
        ...
        // Store pending link
        long expiryTime = System.currentTimeMillis() + (DiscordConfig.CONFIG.linkCodeExpiry.get() * 1000L);
        pendingLinks.put(code, new PendingLink(minecraftUUID, minecraftUsername, expiryTime));

        VonixCore.LOGGER.info("Generated link code {} for player {} ({})", code, minecraftUsername, minecraftUUID);
        return code;
    }
    ...
    public LinkResult verifyAndLink(String code, String discordId, String discordUsername) {
        ...
        VonixCore.LOGGER.info("Successfully linked {} ({}) to Discord user {} ({})",
                pending.minecraftUsername, pending.minecraftUUID, discordUsername, discordId);

        return new LinkResult(true,
                "Successfully linked " + pending.minecraftUsername + " to Discord user " + discordUsername);
    }
    ...
    public boolean unlinkMinecraft(UUID minecraftUUID) {
        ...
        if (removed != null) {
            save();
            VonixCore.LOGGER.info("Unlinked {} ({}) from Discord user {} ({})",
    ...
    public boolean unlinkDiscord(String discordId) {
        ...
        if (toRemove != null) {
            ...
            save();
            VonixCore.LOGGER.info("Unlinked Discord user {} from Minecraft account", discordId);
            return true;
        }
        return false;
    }
    ...
    private void save() {
        try {
            ...
            if (DiscordConfig.CONFIG.debugLogging.get()) {
                VonixCore.LOGGER.debug("Saved {} linked accounts to {}", linkedAccounts.size(), dataFile);
            }
        } catch (IOException e) {
            VonixCore.LOGGER.error("Failed to save linked accounts!", e);
        }
    }
    ...
    private void load() {
        if (!Files.exists(dataFile)) {
            VonixCore.LOGGER.info("No Discord linked accounts file found, starting fresh");
            return;
        }
        ...
        try (Reader reader = new FileReader(dataFile.toFile())) {
            ...
            if (loaded != null) {
                ...
                VonixCore.LOGGER.info("Loaded {} linked accounts from {}", linkedAccounts.size(), dataFile);
            }
        } catch (IOException e) {
            VonixCore.LOGGER.error("Failed to load linked accounts!", e);
        }
    }
