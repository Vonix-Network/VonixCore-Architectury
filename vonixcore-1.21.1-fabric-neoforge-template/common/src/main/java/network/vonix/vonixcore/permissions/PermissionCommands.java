...
public class PermissionCommands {
    ...
    private static int userInfo(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        ctx.getSource().sendSuccess(
                () -> Component.literal("§6=== User: §e" + target.getName().getString() + " §6==="),
                false);
        ctx.getSource().sendSuccess(() -> Component.literal("§7UUID: §f" + uuid), false);
        ctx.getSource().sendSuccess(() -> Component.literal("§7Primary Group: §e" + pm.getPrimaryGroup(uuid)),
                false);
        ctx.getSource().sendSuccess(
                () -> Component.literal("§7Prefix: §f" + pm.getPrefix(uuid).replace("§", "&")), false);
        ctx.getSource().sendSuccess(
                () -> Component.literal("§7Suffix: §f" + pm.getSuffix(uuid).replace("§", "&")), false);
    ...
    private static int userSetGroup(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if (pm.isUsingLuckPerms()) {
            ctx.getSource().sendFailure(Component.literal("§cLuckPerms is active - use /lp command"));
            return 0;
        }

        if (pm.getGroup(groupName) == null) {
            ctx.getSource().sendFailure(Component.literal("§cGroup '" + groupName + "' does not exist"));
            return 0;
        }
    ...
    private static int userAddGroup(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if (pm.isUsingLuckPerms()) {
            ctx.getSource().sendFailure(Component.literal("§cLuckPerms is active"));
            return 0;
        }
        ...
        ctx.getSource().sendSuccess(() -> Component
                .literal("§aAdded §e" + target.getName().getString() + "§a to group §e" + groupName),
                true);
        return 1;
    }
    ...
    private static int userSetPermission(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        ctx.getSource().sendSuccess(() -> Component.literal("§aSet §e" + permission + "§a = " + valStr
    ...
    private static int groupInfo(CommandContext<CommandSourceStack> ctx) {
        ...
        ctx.getSource().sendSuccess(() -> Component.literal("§6=== Group: §e" + group.getName() + " §6==="),
                false);
        ctx.getSource().sendSuccess(() -> Component.literal("§7Display: §f" + group.getDisplayName()), false);
        ctx.getSource().sendSuccess(() -> Component.literal("§7Weight: §e" + group.getWeight()), false);
        ctx.getSource().sendSuccess(
                () -> Component.literal("§7Prefix: §f" + group.getPrefix().replace("§", "&")), false);
        ctx.getSource().sendSuccess(() -> Component
                .literal("§7Parent: §e" + (group.getParent() != null ? group.getParent() : "none")),
                false);
        ctx.getSource().sendSuccess(
                () -> Component.literal("§7Permissions: §f" + group.getPermissions().size()), false);
        return 1;
    }
    ...
    private static int groupCreate(CommandContext<CommandSourceStack> ctx) {
        ...
        ctx.getSource().sendSuccess(() -> Component.literal("§aCreated group §e" + groupName), true);
        return 1;
    }
    ...
    private static int groupSetPermission(CommandContext<CommandSourceStack> ctx) {
        ...
        ctx.getSource().sendSuccess(() -> Component.literal("§aSet §e" + permission + "§a = "
    ...
    private static int groupUnsetPermission(CommandContext<CommandSourceStack> ctx) {
        ...
        saveGroupAsync(group);
        ctx.getSource().sendSuccess(
                () -> Component.literal("§aUnset §e" + permission + "§a for §e" + groupName), true);
        return 1;
    }
    ...
    private static int groupSetPrefix(CommandContext<CommandSourceStack> ctx) {
        ...
        saveGroupAsync(group);
        ctx.getSource().sendSuccess(
                () -> Component.literal("§aSet prefix for §e" + groupName + "§a: " + prefix), true);
        return 1;
    }
    ...
    private static int groupSetSuffix(CommandContext<CommandSourceStack> ctx) {
        ...
        saveGroupAsync(group);
        ctx.getSource().sendSuccess(
                () -> Component.literal("§aSet suffix for §e" + groupName + "§a: " + suffix), true);
        return 1;
    }
    ...
    private static int groupSetWeight(CommandContext<CommandSourceStack> ctx) {
        ...
        saveGroupAsync(group);
        ctx.getSource().sendSuccess(
                () -> Component.literal("§aSet weight for §e" + groupName + "§a: §e" + weight), true);
        return 1;
    }
    ...
    private static int groupSetDisplayName(CommandContext<CommandSourceStack> ctx) {
        ...
        saveGroupAsync(group);
        ctx.getSource().sendSuccess(
                () -> Component.literal("§aSet display name for §e" + groupName + "§a: " + name), true);
        return 1;
    }
    ...
    private static int groupSetParent(CommandContext<CommandSourceStack> ctx) {
        ...
        if (group == null || pm.getGroup(parentName) == null) {
            ctx.getSource().sendFailure(Component.literal("§cGroup not found"));
            return 0;
        }
        group.setParent(parentName);
        saveGroupAsync(group);
        ctx.getSource().sendSuccess(
                () -> Component.literal("§aSet parent §e" + parentName + "§a for §e" + groupName),
                true);
        return 1;
    }
    ...
    private static int groupClearParent(CommandContext<CommandSourceStack> ctx) {
        ...
        saveGroupAsync(group);
        ctx.getSource().sendSuccess(() -> Component.literal("§aCleared parent for §e" + groupName), true);
        return 1;
    }

    private static int listGroups(CommandContext<CommandSourceStack> ctx) {
        ctx.getSource().sendSuccess(() -> Component.literal("§6=== Permission Groups ==="), false);
